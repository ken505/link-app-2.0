...
export function Links(item) {
  return (
    ...
    外
      <div className={classes.linkInnerContent}>
      <div> 中１ a</div>
      <div>中１ b</div>
      </div>
      <div className={classes.linkInnerContent}>中２a  </div>
    </div>
      {ITEMS.map((item) => {
        return (
          <Link href={item.href} key={item.href}>
            <a className={classes.aTag}>
              <div className={classes.aTagInnerGrid}>
                ...
                <div className={classes.linkInnerGrid}>
                  {item.image ? (
                    <div className={classes.linkGlinkInnerContentrid}>{item.image}</div>
                  ) : null}
                  {item.title ? (
                    <h2 className={classes.linkInnerContent}>{item.title}</h2>
                  ) : null}
                </div>
                {item.description ? (
